...
import { ddDelayItemCreateSchema, parseBody } from "@/lib/superintendent/validation";
...
export async function GET(request: Request) {
  ...
  const where: Prisma.DdDelayItemWhereInput = {
    ...notDeleted,
    ...buildChildEntityWhere(dryDockProjectId, guard.projectFilter),
  };
  if (status) where.status = status;

  const [total, delays] = await Promise.all([
  ...
  return NextResponse.json(paginatedResult(delays, total, page, limit));
}
...
export async function POST(request: Request) {
  ...
  const parsed = parseBody(ddDelayItemCreateSchema, await request.json());
  ...
  const delay = await prisma.ddDelayItem.create({ data: parsed.data });
