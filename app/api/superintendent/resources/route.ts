...
import { ddResourceAllocationCreateSchema, parseBody } from "@/lib/superintendent/validation";
...
export async function GET(request: Request) {
  ...
  const where: Prisma.DdResourceAllocationWhereInput = {
    ...notDeleted,
    ...buildChildEntityWhere(dryDockProjectId, guard.projectFilter),
  };
  if (status) where.status = status as Prisma.EnumDdResourceStatusFilter["equals"];
...
export async function POST(request: Request) {
  ...
  const parsed = parseBody(ddResourceAllocationCreateSchema, await request.json());
  ...
  const resource = await prisma.ddResourceAllocation.create({ data: parsed.data });
