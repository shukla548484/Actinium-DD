...
import { ddSurveyItemCreateSchema, parseBody } from "@/lib/superintendent/validation";
...
export async function GET(request: Request) {
  ...
  const where: Prisma.DdSurveyItemWhereInput = {
    ...notDeleted,
    ...buildChildEntityWhere(dryDockProjectId, guard.projectFilter),
  };
  if (status) where.status = status as Prisma.EnumDdSurveyStatusFilter["equals"];

  const [total, surveyItems] = await Promise.all([
  ...
  return NextResponse.json(paginatedResult(surveyItems, total, page, limit));
}
...
export async function POST(request: Request) {
  ...
  const parsed = parseBody(ddSurveyItemCreateSchema, await request.json());
  ...
  const surveyItem = await prisma.ddSurveyItem.create({ data: parsed.data });
  return NextResponse.json({ surveyItem }, { status: 201 });
}
