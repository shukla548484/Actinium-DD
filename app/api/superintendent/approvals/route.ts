...
import { ddApprovalRequestCreateSchema, parseBody } from "@/lib/superintendent/validation";
...
export async function GET(request: Request) {
  ...
  const where: Prisma.DdApprovalRequestWhereInput = {
    ...notDeleted,
    ...buildChildEntityWhere(dryDockProjectId, guard.projectFilter),
  };
  if (status) where.status = status as Prisma.EnumDdApprovalStatusFilter["equals"];
  ...
  return NextResponse.json(paginatedResult(approvals, total, page, limit));
}
...
export async function POST(request: Request) {
  ...
  const parsed = parseBody(ddApprovalRequestCreateSchema, await request.json());
  ...
  const approval = await prisma.ddApprovalRequest.create({ data: parsed.data });
