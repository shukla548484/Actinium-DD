...
import { ddRiskItemCreateSchema, parseBody } from "@/lib/superintendent/validation";
...
export async function GET(request: Request) {
  ...
  const where: Prisma.DdRiskItemWhereInput = {
    ...notDeleted,
  ...
  return NextResponse.json(paginatedResult(risks, total, page, limit));
}
...
export async function POST(request: Request) {
  ...
  const parsed = parseBody(ddRiskItemCreateSchema, await request.json());
  ...
  const risk = await prisma.ddRiskItem.create({ data: parsed.data });
