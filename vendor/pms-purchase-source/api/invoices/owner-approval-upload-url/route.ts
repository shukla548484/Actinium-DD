...
import {
  ...
  buildPurchaseOwnerApprovalGcsPath,
...
/**
 * POST /api/invoices/owner-approval-upload-url
 * Signed GCS URL for owner approval attachment on invoice upload/edit.
 */
export async function POST(request: NextRequest) {
  try {
    ...
    if (!purchaseOrderId || !fileName) {
      return NextResponse.json(
        { error: "purchaseOrderId and fileName are required" },
        { status: 400 }
      );
    }
    ...
    const storageFileName = buildOwnerApprovalStorageFileName(invoiceNumber, fileName);
    const { gcsPath, fileUrl } = buildPurchaseOwnerApprovalGcsPath({
    ...
    console.error("Error getting owner approval upload URL:", error);
    return NextResponse.json(
      {
        error: "Failed to get owner approval upload URL",
