...
export const V311_DMK_MACHINERY_FAMILY = "Deck Machinery – Windlass / Winches / Capstans";
...
export function isV311DeckMachineryWorkbook(workbook: XLSX.WorkBook): boolean {
  const rows = sheetRows(workbook, MASTER_SHEET);
  const code = cellStr(rows[0]?.["Job Code"]);
  return code.startsWith("DMK-");
}

export function normalizeV311DmkJobCode(raw: string): string {
...
function normalizeV311DmkJobRows(rows: Array<Record<string, unknown>>): Array<Record<string, unknown>> {
  const systemCodes = new Map<string, string>();
  let systemIndex = 0;

  return rows.map((row) => {
    const jobId = normalizeV311DmkJobCode(cellStr(row["Job Code"]));
    ...
    const machinery = cellStr(row["Machinery Family"]) || V311_DMK_MACHINERY_FAMILY;
    const dryDock = dryDockFlag(row["Dry Dock Scope"]);
    return {
      "Job ID": jobId,
      Release: EMDR_V312_RELEASE,
      Department: "Deck",
      Machinery: /deck machinery/i.test(machinery) ? V311_DMK_MACHINERY_FAMILY : machinery,
      System: systemName,
      ...
      "RFQ Category": V311_DMK_MACHINERY_FAMILY,
...
function synthesizeV311DmkMeasurements(masterJobs: ParsedMasterJobRow[]): ParsedMeasurementRow[] {
...
function synthesizeV311DmkChecklist(masterJobs: ParsedMasterJobRow[]): ParsedChecklistRow[] {
  return masterJobs.map((job, index) => {
    const ids = idsFromJobId(job.jobId);
    return {
      rowNumber: index + 2,
...
function synthesizeV311DmkRfq(masterJobs: ParsedMasterJobRow[]): ParsedRfqRow[] {
  return masterJobs.map((job, index) => {
    const budgetCategory = job.budgetCategory || "Deck Machinery Jobs Cost";
    return {
      rowNumber: index + 2,
...
function synthesizeV311DmkEquipmentMaster(jobs: ParsedMasterJobRow[]): ParsedEquipmentMasterRow[] {
...
function synthesizeV311DmkComponentMaster(jobs: ParsedMasterJobRow[]): ParsedComponentMasterRow[] {
  const seen = new Set<string>();
...
function buildV311DmkRepositoryIndex(jobs: ParsedMasterJobRow[]): V3RepositoryIndexRow[] {
  ...
  return [...bySystem.entries()].map(([systemName, entry], index) => ({
    ...
    status: "Completed",
    machineryFamily: V311_DMK_MACHINERY_FAMILY as V3RepositoryIndexRow["machineryFamily"],
  }));
}

export function parseV311DeckMachineryRepositoryBuffer(buffer: ArrayBuffer | Uint8Array): ParsedV3MasterRepository {
  const workbook = XLSX.read(buffer, { type: "array" });
  if (!isV311DeckMachineryWorkbook(workbook)) {
    throw new Error("Not a V3.11 Deck Machinery typewise EMDR repository workbook");
  }

  const jobRows = normalizeV311DmkJobRows(sheetRows(workbook, MASTER_SHEET));
  ...
  const parsed: ParsedMtilWorkbook = {
    ...
    masterJobs,
    templates,
    measurements: synthesizeV311DmkMeasurements(masterJobs),
    checklistItems: synthesizeV311DmkChecklist(masterJobs),
    scopeSteps,
    attachments: [],
    spares: [],
    rfqMappings: synthesizeV311DmkRfq(masterJobs),
    workflows,
  };
  ...
  return {
    ...normalized,
    emdrMasterData: {
      equipmentMaster: synthesizeV311DmkEquipmentMaster(normalized.masterJobs),
      componentMaster: synthesizeV311DmkComponentMaster(normalized.masterJobs),
      tools: [],
    },
    repositoryIndex: buildV311DmkRepositoryIndex(normalized.masterJobs),
    release: EMDR_V312_RELEASE,
  };
}

export function parseV311DeckMachineryRepositoryFile(path: string): ParsedV3MasterRepository {
  const bytes = fs.readFileSync(path);
  return parseV311DeckMachineryRepositoryBuffer(bytes);
}

export function parseV311DeckMachineryRepositoryIfExists(path: string): ParsedV3MasterRepository | null {
  if (!fs.existsSync(path)) return null;
  try {
    return parseV311DeckMachineryRepositoryFile(path);
  } catch {
    return null;
  }
}
