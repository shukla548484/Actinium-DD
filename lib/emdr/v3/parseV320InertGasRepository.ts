...
export const V320_IGS_MACHINERY_FAMILY = "Inert Gas System / IGG / Nitrogen Generator";
...
export function isV320InertGasWorkbook(workbook: XLSX.WorkBook): boolean {
  const rows = sheetRows(workbook, MASTER_SHEET);
  const code = cellStr(rows[0]?.["Job Code"]);
  return code.startsWith("IGS-");
}

export function normalizeV320IgsJobCode(raw: string): string {
...
function riskLevelFromRow(row: Record<string, unknown>): string {
  const criticality = cellStr(row["Job Criticality"] ?? row["Criticality"]);
...
function detailedScopeFromRow(row: Record<string, unknown>): string {
  ...
  if (!desc || desc.length < 12) return action || desc;
  return desc;
}
...
function systemNameFromRow(row: Record<string, unknown>): string {
  ...
  return V320_IGS_MACHINERY_FAMILY;
}

function normalizeV320IgsJobRows(rows: Array<Record<string, unknown>>): Array<Record<string, unknown>> {
  const systemCodes = new Map<string, string>();
  let systemIndex = 0;

  return rows.map((row) => {
    const jobId = normalizeV320IgsJobCode(cellStr(row["Job Code"]));
    ...
    const machinery = cellStr(row["Machinery Family"]) || V320_IGS_MACHINERY_FAMILY;
    const dryDock = dryDockFlag(row["Routine / Dry Dock"]);
    return {
      "Job ID": jobId,
      Release: EMDR_V312_RELEASE,
      Department: "Engine",
      Machinery: /inert gas|igg|nitrogen/i.test(machinery) ? V320_IGS_MACHINERY_FAMILY : machinery,
      System: systemName,
      Component: cellStr(row["Component / Asset"]),
      ...
      Workshop: "engine room / inert gas / nitrogen generator",
      ...
      "RFQ Category": V320_IGS_MACHINERY_FAMILY,
      "Budget Category": cellStr(row["Section"]) || "Inert Gas / IGG / Nitrogen",
...
function synthesizeV320IgsMeasurements(masterJobs: ParsedMasterJobRow[]): ParsedMeasurementRow[] {
...
function synthesizeV320IgsChecklist(masterJobs: ParsedMasterJobRow[]): ParsedChecklistRow[] {
  return masterJobs.map((job, index) => {
    const ids = idsFromJobId(job.jobId);
    return {
      rowNumber: index + 2,
...
function synthesizeV320IgsRfq(masterJobs: ParsedMasterJobRow[]): ParsedRfqRow[] {
  return masterJobs.map((job, index) => {
    const budgetCategory = job.budgetCategory || "Inert Gas / IGG / Nitrogen";
    return {
      rowNumber: index + 2,
...
function synthesizeV320IgsEquipmentMaster(jobs: ParsedMasterJobRow[]): ParsedEquipmentMasterRow[] {
...
function synthesizeV320IgsComponentMaster(jobs: ParsedMasterJobRow[]): ParsedComponentMasterRow[] {
  const seen = new Set<string>();
...
function buildV320IgsRepositoryIndex(jobs: ParsedMasterJobRow[]): V3RepositoryIndexRow[] {
  ...
  return [...bySystem.entries()].map(([systemName, entry], index) => ({
    ...
    status: "Completed",
    machineryFamily: V320_IGS_MACHINERY_FAMILY as V3RepositoryIndexRow["machineryFamily"],
  }));
}

export function parseV320InertGasRepositoryBuffer(buffer: ArrayBuffer | Uint8Array): ParsedV3MasterRepository {
  const workbook = XLSX.read(buffer, { type: "array" });
  if (!isV320InertGasWorkbook(workbook)) {
    throw new Error("Not a V3.20 Inert Gas / IGG / Nitrogen System typewise EMDR repository workbook");
  }

  const jobRows = normalizeV320IgsJobRows(sheetRows(workbook, MASTER_SHEET));
  ...
  const parsed: ParsedMtilWorkbook = {
    ...
    masterJobs,
    templates,
    measurements: synthesizeV320IgsMeasurements(masterJobs),
    checklistItems: synthesizeV320IgsChecklist(masterJobs),
    scopeSteps,
    attachments: [],
    spares: [],
    rfqMappings: synthesizeV320IgsRfq(masterJobs),
    workflows,
  };
  ...
  return {
    ...normalized,
    emdrMasterData: {
      equipmentMaster: synthesizeV320IgsEquipmentMaster(normalized.masterJobs),
      componentMaster: synthesizeV320IgsComponentMaster(normalized.masterJobs),
      tools: [],
    },
    repositoryIndex: buildV320IgsRepositoryIndex(normalized.masterJobs),
    release: EMDR_V312_RELEASE,
  };
}

export function parseV320InertGasRepositoryFile(path: string): ParsedV3MasterRepository {
  const bytes = fs.readFileSync(path);
  return parseV320InertGasRepositoryBuffer(bytes);
}

export function parseV320InertGasRepositoryIfExists(path: string): ParsedV3MasterRepository | null {
  if (!fs.existsSync(path)) return null;
  try {
    return parseV320InertGasRepositoryFile(path);
  } catch {
    return null;
  }
}
