...
export const V315_PCS_MACHINERY_FAMILY = "Purifiers / Centrifugal Separators";
...
export function isV315PurifiersWorkbook(workbook: XLSX.WorkBook): boolean {
  const rows = sheetRows(workbook, MASTER_SHEET);
  const code = cellStr(rows[0]?.["Job Code"]);
  return code.startsWith("PCS-");
}

export function normalizeV315PcsJobCode(raw: string): string {
...
function normalizeV315PcsJobRows(rows: Array<Record<string, unknown>>): Array<Record<string, unknown>> {
  const systemCodes = new Map<string, string>();
  let systemIndex = 0;

  return rows.map((row) => {
    const jobId = normalizeV315PcsJobCode(cellStr(row["Job Code"]));
    ...
    const machinery = cellStr(row["Machinery Family"]) || V315_PCS_MACHINERY_FAMILY;
    const dryDock = dryDockFlag(row["Dry Dock Scope"]);
    return {
      "Job ID": jobId,
      Release: EMDR_V312_RELEASE,
      Department: "Engine",
      Machinery: /purifier|centrifugal|separator/i.test(machinery) ? V315_PCS_MACHINERY_FAMILY : machinery,
      System: systemName,
      ...
      Workshop: "engine room / purifier room / workshop",
      "Responsible Vessel Role": cellStr(row["PIC"]) || "Third Engineer",
      "Review Role": cellStr(row["Verifying Authority"]) || "Second Engineer",
      ...
      "RFQ Category": V315_PCS_MACHINERY_FAMILY,
      "Budget Category": cellStr(row["Section"]) || "Purifiers / Centrifugal Separators",
...
function synthesizeV315PcsMeasurements(masterJobs: ParsedMasterJobRow[]): ParsedMeasurementRow[] {
...
function synthesizeV315PcsChecklist(masterJobs: ParsedMasterJobRow[]): ParsedChecklistRow[] {
  return masterJobs.map((job, index) => {
    const ids = idsFromJobId(job.jobId);
    return {
      rowNumber: index + 2,
...
function synthesizeV315PcsRfq(masterJobs: ParsedMasterJobRow[]): ParsedRfqRow[] {
  return masterJobs.map((job, index) => {
    const budgetCategory = job.budgetCategory || "Purifiers / Centrifugal Separators";
    return {
      rowNumber: index + 2,
...
function synthesizeV315PcsEquipmentMaster(jobs: ParsedMasterJobRow[]): ParsedEquipmentMasterRow[] {
...
function synthesizeV315PcsComponentMaster(jobs: ParsedMasterJobRow[]): ParsedComponentMasterRow[] {
  const seen = new Set<string>();
...
function buildV315PcsRepositoryIndex(jobs: ParsedMasterJobRow[]): V3RepositoryIndexRow[] {
  ...
  return [...bySystem.entries()].map(([systemName, entry], index) => ({
    ...
    status: "Completed",
    machineryFamily: V315_PCS_MACHINERY_FAMILY as V3RepositoryIndexRow["machineryFamily"],
  }));
}

export function parseV315PurifiersRepositoryBuffer(buffer: ArrayBuffer | Uint8Array): ParsedV3MasterRepository {
  const workbook = XLSX.read(buffer, { type: "array" });
  if (!isV315PurifiersWorkbook(workbook)) {
    throw new Error("Not a V3.15 Purifiers typewise EMDR repository workbook");
  }

  const jobRows = normalizeV315PcsJobRows(sheetRows(workbook, MASTER_SHEET));
  ...
  const parsed: ParsedMtilWorkbook = {
    ...
    masterJobs,
    templates,
    measurements: synthesizeV315PcsMeasurements(masterJobs),
    checklistItems: synthesizeV315PcsChecklist(masterJobs),
    scopeSteps,
    attachments: [],
    spares: [],
    rfqMappings: synthesizeV315PcsRfq(masterJobs),
    workflows,
  };
  ...
  return {
    ...normalized,
    emdrMasterData: {
      equipmentMaster: synthesizeV315PcsEquipmentMaster(normalized.masterJobs),
      componentMaster: synthesizeV315PcsComponentMaster(normalized.masterJobs),
      tools: [],
    },
    repositoryIndex: buildV315PcsRepositoryIndex(normalized.masterJobs),
    release: EMDR_V312_RELEASE,
  };
}

export function parseV315PurifiersRepositoryFile(path: string): ParsedV3MasterRepository {
  const bytes = fs.readFileSync(path);
  return parseV315PurifiersRepositoryBuffer(bytes);
}

export function parseV315PurifiersRepositoryIfExists(path: string): ParsedV3MasterRepository | null {
  if (!fs.existsSync(path)) return null;
  try {
    return parseV315PurifiersRepositoryFile(path);
  } catch {
    return null;
  }
}
