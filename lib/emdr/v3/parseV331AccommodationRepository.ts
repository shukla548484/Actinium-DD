...
export const V331_AGLH_MACHINERY_FAMILY =
  "Accommodation / Galley / Laundry / Hotel Equipment";

const AGLH_JOB_CODE_PATTERN = /^AGLH-\d+$/i;

export function isV331TypewiseAglhJobId(jobId: string): boolean {
  return /^JOBS-AGLH-\d+$/i.test(jobId);
}
...
function resolveMasterSheetName(workbook: XLSX.WorkBook): string | null {
  ...
  return (
    workbook.SheetNames.find((name) =>
      /accommodation|galley|laundry|hotel|job.?repo/i.test(name),
    ) ?? null
  );
}
...
export function isV331AccommodationWorkbook(workbook: XLSX.WorkBook): boolean {
  ...
  return rows.some((row) => {
    ...
    return (
      /^V3\.31-/i.test(jobCode) ||
      /accommodation|galley|laundry|hotel/i.test(machineryGroup) ||
      /accommodation|galley|laundry|hotel|sanitary|provision refrigeration|waste handling/i.test(
        equipmentSystem,
      )
    );
  });
...
export function normalizeV331AglhJobCode(raw: string): string {
  const code = cellStr(raw).replace(/^JOBS-/, "");
  if (AGLH_JOB_CODE_PATTERN.test(code)) return `JOBS-${code.toUpperCase()}`;
  const v331Prefixed = /^V3\.31-AGLH-(\d+)$/i.exec(code);
  if (v331Prefixed) return `JOBS-AGLH-${v331Prefixed[1].padStart(4, "0")}`;
  const v331Plain = /^V3\.31-(\d+)$/i.exec(code);
  if (v331Plain) return `JOBS-AGLH-${v331Plain[1].padStart(4, "0")}`;
  return "";
}

function formatAglhJobCode(sequence: number): string {
  return `JOBS-AGLH-${String(sequence).padStart(4, "0")}`;
}
...
function picFromRow(row: Record<string, unknown>): string {
  ...
  if (lower.includes("reefer engineer") || lower.includes("duty engineer")) return "Second Engineer";
  ...
  if (lower.includes("chief cook") || lower.includes("second cook")) return "Second Engineer";
...
function departmentFromRow(row: Record<string, unknown>): string {
  ...
  if (pic.includes("chief cook") || pic.includes("second cook") || /galley|laundry|accommodation|hotel|sanitary/i.test(system)) {
    return "Accommodation";
  }
  if (/safety interface|hull\/safety/i.test(system)) return "Safety";
  if (/electrical interface/i.test(system)) return "Electrical";
  return "Engine";
}
...
function systemNameFromRow(row: Record<string, unknown>): string {
  ...
  return asset || V331_AGLH_MACHINERY_FAMILY;
}
...
function componentFromRow(row: Record<string, unknown>): string {
  ...
  return asset || "Accommodation / Hotel Equipment";
}
...
function workshopFromRow(row: Record<string, unknown>): string {
  ...
  if (/electrical interface|lighting|switch|power/i.test(combined)) return "electrical workshop";
  if (/safety interface|fire door|emergency light|pa\/ga|galley fire/i.test(combined)) {
    return "safety workshop";
  }
  if (/provision refrigeration|cold room|refrigerat|ice maker|medical refrig/i.test(combined)) {
    return "machinery workshop";
  }
  if (/hvac|ventilation|air handling/i.test(combined)) return "engine room / hvac ventilation";
  if (/hull|sanitary|waste|garbage|compactor/i.test(combined)) return "hull workshop";
  return "accommodation workshop";
}
...
function componentTypeFromSystem(systemName: string, component: string): string {
  ...
  if (/galley/i.test(combined)) return "Galley Equipment";
  if (/laundry/i.test(combined)) return "Laundry Equipment";
  if (/provision refrigeration|cold room|refrigerat|ice maker/i.test(combined)) {
    return "Provision Refrigeration";
  }
  if (/sanitary|hydrophore|hot water/i.test(combined)) return "Sanitary System";
  if (/waste|garbage|compactor|disposer/i.test(combined)) return "Waste Handling";
  if (/safety|fire door|emergency|pa\/ga/i.test(combined)) return "Safety Interface";
  if (/electrical/i.test(combined)) return "Electrical Interface";
  if (/hvac|ventilation/i.test(combined)) return "Accommodation HVAC";
  if (/accommodation management|hotel/i.test(combined)) return "Accommodation Management";
  return "Accommodation Equipment";
}

function normalizeV331AglhJobRows(rows: Array<Record<string, unknown>>): Array<Record<string, unknown>> {
  ...
  return rows.flatMap((row) => {
    ...
    const explicitCode = normalizeV331AglhJobCode(cellStr(row["Job Code"]));
    sequence += 1;
    let jobId = explicitCode || formatAglhJobCode(sequence);
    if (seenJobIds.has(jobId)) {
      sequence += 1;
      jobId = formatAglhJobCode(sequence);
    }
    ...
    return [
      {
        ...
        Machinery: V331_AGLH_MACHINERY_FAMILY,
        System: systemName,
        ...
        "RFQ Category": cellStr(row["RFQ Cost Category"]) || V331_AGLH_MACHINERY_FAMILY,
...
function synthesizeV331AglhMeasurements(masterJobs: ParsedMasterJobRow[]): ParsedMeasurementRow[] {
...
function synthesizeV331AglhChecklist(masterJobs: ParsedMasterJobRow[]): ParsedChecklistRow[] {
  return masterJobs.map((job, index) => {
    const ids = idsFromJobId(job.jobId);
    return {
      rowNumber: index + 2,
...
function synthesizeV331AglhRfq(masterJobs: ParsedMasterJobRow[]): ParsedRfqRow[] {
  return masterJobs.map((job) => {
    const budgetCategory = job.budgetCategory || V331_AGLH_MACHINERY_FAMILY;
    return {
      rowNumber: 0,
...
function synthesizeV331AglhEquipmentMaster(jobs: ParsedMasterJobRow[]): ParsedEquipmentMasterRow[] {
...
function synthesizeV331AglhComponentMaster(jobs: ParsedMasterJobRow[]): ParsedComponentMasterRow[] {
  const seen = new Set<string>();
...
function buildV331AglhRepositoryIndex(jobs: ParsedMasterJobRow[]): V3RepositoryIndexRow[] {
  ...
  return [...bySystem.entries()].map(([systemName, entry], index) => ({
    ...
    status: "Completed",
    machineryFamily: V331_AGLH_MACHINERY_FAMILY as V3RepositoryIndexRow["machineryFamily"],
  }));
}

export function parseV331AccommodationRepositoryBuffer(
  buffer: ArrayBuffer | Uint8Array,
): ParsedV3MasterRepository {
  const workbook = XLSX.read(buffer, { type: "array" });
  if (!isV331AccommodationWorkbook(workbook)) {
    throw new Error(
      "Not a V3.31 Accommodation / Galley / Laundry / Hotel Equipment EMDR repository workbook",
    );
  }

  const sheetName = resolveMasterSheetName(workbook);
  if (!sheetName) {
    throw new Error("V3.31 Accommodation workbook is missing a recognizable jobs sheet");
  }

  const jobRows = normalizeV331AglhJobRows(sheetRows(workbook, sheetName));
  ...
  const parsed: ParsedMtilWorkbook = {
    ...
    masterJobs,
    templates,
    measurements: synthesizeV331AglhMeasurements(masterJobs),
    checklistItems: synthesizeV331AglhChecklist(masterJobs),
    scopeSteps,
    attachments: [],
    spares: [],
    rfqMappings: synthesizeV331AglhRfq(masterJobs),
    workflows,
  };
  ...
  return {
    ...normalized,
    emdrMasterData: {
      equipmentMaster: synthesizeV331AglhEquipmentMaster(normalized.masterJobs),
      componentMaster: synthesizeV331AglhComponentMaster(normalized.masterJobs),
      tools: [],
    },
    repositoryIndex: buildV331AglhRepositoryIndex(normalized.masterJobs),
    release: EMDR_V312_RELEASE,
  };
}

export function parseV331AccommodationRepositoryFile(path: string): ParsedV3MasterRepository {
  const bytes = fs.readFileSync(path);
  return parseV331AccommodationRepositoryBuffer(bytes);
}

export function parseV331AccommodationRepositoryIfExists(
  path: string,
): ParsedV3MasterRepository | null {
  if (!fs.existsSync(path)) return null;
  try {
    return parseV331AccommodationRepositoryFile(path);
  } catch {
    return null;
  }
}
