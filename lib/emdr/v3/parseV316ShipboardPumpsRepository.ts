...
const MASTER_SHEET = "Pump_Jobs";

export const V316_PUMP_MACHINERY_FAMILY = "Shipboard Pumps";
...
export function isV316ShipboardPumpsWorkbook(workbook: XLSX.WorkBook): boolean {
  const rows = sheetRows(workbook, MASTER_SHEET);
  const code = cellStr(rows[0]?.["Job Code"]);
  return code.startsWith("PUMP-");
}

export function normalizeV316PumpJobCode(raw: string): string {
...
function systemNameFromRow(row: Record<string, unknown>): string {
  ...
  const equipment = cellStr(row["Pump / Equipment"]);
  if (equipment) return equipment;
  return "Shipboard Pumps";
}

function normalizeV316PumpJobRows(rows: Array<Record<string, unknown>>): Array<Record<string, unknown>> {
  const systemCodes = new Map<string, string>();
  let systemIndex = 0;

  return rows.map((row) => {
    const jobId = normalizeV316PumpJobCode(cellStr(row["Job Code"]));
    ...
    const machinery = cellStr(row["Machinery Family"]) || V316_PUMP_MACHINERY_FAMILY;
    const dryDock = dryDockFlag(row["Dry Dock / PMS"] ?? row["Dry Dock Scope"]);
    return {
      "Job ID": jobId,
      Release: EMDR_V312_RELEASE,
      Department: "Engine",
      Machinery: /shipboard pump/i.test(machinery) ? V316_PUMP_MACHINERY_FAMILY : machinery,
      System: systemName,
      ...
      Workshop: "engine room / pump room / workshop",
      ...
      "RFQ Category": V316_PUMP_MACHINERY_FAMILY,
      "Budget Category": cellStr(row["Section"]) || "Shipboard Pumps",
...
function synthesizeV316PumpMeasurements(masterJobs: ParsedMasterJobRow[]): ParsedMeasurementRow[] {
...
function synthesizeV316PumpChecklist(masterJobs: ParsedMasterJobRow[]): ParsedChecklistRow[] {
  return masterJobs.map((job, index) => {
    const ids = idsFromJobId(job.jobId);
    return {
      rowNumber: index + 2,
...
function synthesizeV316PumpRfq(masterJobs: ParsedMasterJobRow[]): ParsedRfqRow[] {
  return masterJobs.map((job, index) => {
    const budgetCategory = job.budgetCategory || "Shipboard Pumps";
    return {
      rowNumber: index + 2,
...
function synthesizeV316PumpEquipmentMaster(jobs: ParsedMasterJobRow[]): ParsedEquipmentMasterRow[] {
...
function synthesizeV316PumpComponentMaster(jobs: ParsedMasterJobRow[]): ParsedComponentMasterRow[] {
  const seen = new Set<string>();
...
function buildV316PumpRepositoryIndex(jobs: ParsedMasterJobRow[]): V3RepositoryIndexRow[] {
  ...
  return [...bySystem.entries()].map(([systemName, entry], index) => ({
    ...
    status: "Completed",
    machineryFamily: V316_PUMP_MACHINERY_FAMILY as V3RepositoryIndexRow["machineryFamily"],
  }));
}

export function parseV316ShipboardPumpsRepositoryBuffer(buffer: ArrayBuffer | Uint8Array): ParsedV3MasterRepository {
  const workbook = XLSX.read(buffer, { type: "array" });
  if (!isV316ShipboardPumpsWorkbook(workbook)) {
    throw new Error("Not a V3.16 Shipboard Pumps typewise EMDR repository workbook");
  }

  const jobRows = normalizeV316PumpJobRows(sheetRows(workbook, MASTER_SHEET));
  ...
  const parsed: ParsedMtilWorkbook = {
    ...
    masterJobs,
    templates,
    measurements: synthesizeV316PumpMeasurements(masterJobs),
    checklistItems: synthesizeV316PumpChecklist(masterJobs),
    scopeSteps,
    attachments: [],
    spares: [],
    rfqMappings: synthesizeV316PumpRfq(masterJobs),
    workflows,
  };
  ...
  return {
    ...normalized,
    emdrMasterData: {
      equipmentMaster: synthesizeV316PumpEquipmentMaster(normalized.masterJobs),
      componentMaster: synthesizeV316PumpComponentMaster(normalized.masterJobs),
      tools: [],
    },
    repositoryIndex: buildV316PumpRepositoryIndex(normalized.masterJobs),
    release: EMDR_V312_RELEASE,
  };
}

export function parseV316ShipboardPumpsRepositoryFile(path: string): ParsedV3MasterRepository {
  const bytes = fs.readFileSync(path);
  return parseV316ShipboardPumpsRepositoryBuffer(bytes);
}

export function parseV316ShipboardPumpsRepositoryIfExists(path: string): ParsedV3MasterRepository | null {
  if (!fs.existsSync(path)) return null;
  try {
    return parseV316ShipboardPumpsRepositoryFile(path);
  } catch {
    return null;
  }
}
