...
const MASTER_SHEET_CANDIDATES = [
  "Valves_Piping_SeaChest_Jobs",
  "Job_List",
  "PMS_Jobs",
] as const;

export const V327_VPSO_MACHINERY_FAMILY =
  "Valves / Piping / Sea Chest / Overboard Systems";

const VPSO_JOB_CODE_PATTERN = /^VPSO-\d+$/i;

export function isV327TypewiseVpsoJobId(jobId: string): boolean {
  return /^JOBS-VPSO-\d+$/i.test(jobId);
}
...
function resolveMasterSheetName(workbook: XLSX.WorkBook): string | null {
  ...
  return (
    workbook.SheetNames.find((name) => /valves?.*piping|sea chest|overboard/i.test(name)) ?? null
  );
}

/** Scan for the header row — V3.27 workbooks may have title rows above column headers. */
...
export function isV327ValvesPipingWorkbook(workbook: XLSX.WorkBook): boolean {
  ...
  return rows.some((row) => {
    ...
    const machinery = cellStr(row["Machinery"]);
    return (
      /^V3\.27$/i.test(version) &&
      (/valves?.*piping|sea chest|overboard/i.test(section) ||
        /valves?.*piping|sea chest|overboard/i.test(machinery))
    );
  });
}

export function normalizeV327VpsoJobCode(raw: string): string {
  const code = cellStr(raw).replace(/^JOBS-/, "");
  if (!VPSO_JOB_CODE_PATTERN.test(code)) return "";
  return `JOBS-${code.toUpperCase()}`;
}

function formatVpsoJobCode(sequence: number): string {
  return `JOBS-VPSO-${String(sequence).padStart(4, "0")}`;
}
...
function systemNameFromRow(row: Record<string, unknown>): string {
  ...
  const asset = cellStr(row["Equipment / Asset"]);
  return asset || V327_VPSO_MACHINERY_FAMILY;
}
...
function componentFromRow(row: Record<string, unknown>): string {
  ...
  const asset = cellStr(row["Equipment / Asset"]);
  return asset || "Valve / Piping Component";
}
...
function workshopFromRow(row: Record<string, unknown>): string {
  ...
  if (/sea chest|overboard|shell|grating|anode|mgps/i.test(combined)) return "hull workshop";
  if (/pipe|flange|support|expansion|hose|fitting/i.test(combined)) return "pipe / machinery workshop";
...
function componentTypeFromSystem(systemName: string, component: string): string {
  ...
  if (/sea chest|sea suction|grating/i.test(combined)) return "Sea Chest / Sea Suction";
  if (/overboard|shell valve|storm|scupper/i.test(combined)) return "Overboard / Shell Valve";
  if (/pipe|flange|support|expansion|hose|fitting|strainer|drain|vent/i.test(combined)) {
    return "Piping / Fittings";
  }
  if (/control|actuat|remote|pneumatic|hydraulic|electric/i.test(combined)) return "Actuated / Control Valve";
  if (/prv|relief|safety/i.test(combined)) return "Pressure / Safety Valve";
  return "Valve";
}

function normalizeV327VpsoJobRows(rows: Array<Record<string, unknown>>): Array<Record<string, unknown>> {
  ...
  return rows.flatMap((row) => {
    ...
    const explicitCode = normalizeV327VpsoJobCode(cellStr(row["Job Code"]));
    sequence += 1;
    const jobId = explicitCode || formatVpsoJobCode(sequence);
    ...
    return [
      {
        ...
        Department: "Engine",
        Machinery: V327_VPSO_MACHINERY_FAMILY,
        System: systemName,
        ...
        "RFQ Category": V327_VPSO_MACHINERY_FAMILY,
...
function synthesizeV327VpsoMeasurements(masterJobs: ParsedMasterJobRow[]): ParsedMeasurementRow[] {
...
function synthesizeV327VpsoChecklist(masterJobs: ParsedMasterJobRow[]): ParsedChecklistRow[] {
  return masterJobs.map((job, index) => {
    const ids = idsFromJobId(job.jobId);
    return {
      rowNumber: index + 2,
...
function synthesizeV327VpsoRfq(masterJobs: ParsedMasterJobRow[]): ParsedRfqRow[] {
  return masterJobs.map((job) => {
    const budgetCategory = job.budgetCategory || "Valves / Piping / Sea Chest / Overboard";
    return {
      rowNumber: 0,
...
function synthesizeV327VpsoEquipmentMaster(jobs: ParsedMasterJobRow[]): ParsedEquipmentMasterRow[] {
...
function synthesizeV327VpsoComponentMaster(jobs: ParsedMasterJobRow[]): ParsedComponentMasterRow[] {
  const seen = new Set<string>();
...
function buildV327VpsoRepositoryIndex(jobs: ParsedMasterJobRow[]): V3RepositoryIndexRow[] {
  ...
  return [...bySystem.entries()].map(([systemName, entry], index) => ({
    ...
    status: "Completed",
    machineryFamily: V327_VPSO_MACHINERY_FAMILY as V3RepositoryIndexRow["machineryFamily"],
  }));
}

export function parseV327ValvesPipingRepositoryBuffer(
  buffer: ArrayBuffer | Uint8Array,
): ParsedV3MasterRepository {
  const workbook = XLSX.read(buffer, { type: "array" });
  if (!isV327ValvesPipingWorkbook(workbook)) {
    throw new Error(
      "Not a V3.27 Valves / Piping / Sea Chest / Overboard Systems EMDR repository workbook",
    );
  }

  const sheetName = resolveMasterSheetName(workbook);
  if (!sheetName) {
    throw new Error("V3.27 Valves / Piping workbook is missing a recognizable jobs sheet");
  }

  const jobRows = normalizeV327VpsoJobRows(sheetRows(workbook, sheetName));
  ...
  const parsed: ParsedMtilWorkbook = {
    ...
    masterJobs,
    templates,
    measurements: synthesizeV327VpsoMeasurements(masterJobs),
    checklistItems: synthesizeV327VpsoChecklist(masterJobs),
    scopeSteps,
    attachments: [],
    spares: [],
    rfqMappings: synthesizeV327VpsoRfq(masterJobs),
    workflows,
  };
  ...
  return {
    ...normalized,
    emdrMasterData: {
      equipmentMaster: synthesizeV327VpsoEquipmentMaster(normalized.masterJobs),
      componentMaster: synthesizeV327VpsoComponentMaster(normalized.masterJobs),
      tools: [],
    },
    repositoryIndex: buildV327VpsoRepositoryIndex(normalized.masterJobs),
    release: EMDR_V312_RELEASE,
  };
}

export function parseV327ValvesPipingRepositoryFile(path: string): ParsedV3MasterRepository {
  const bytes = fs.readFileSync(path);
  return parseV327ValvesPipingRepositoryBuffer(bytes);
}

export function parseV327ValvesPipingRepositoryIfExists(
  path: string,
): ParsedV3MasterRepository | null {
  if (!fs.existsSync(path)) return null;
  try {
    return parseV327ValvesPipingRepositoryFile(path);
  } catch {
    return null;
  }
}
