...
export const V312_MACHINERY_FAMILY = "Inert Gas / IGG / Scrubber System";
...
export function isV312IncrementalWorkbook(workbook: XLSX.WorkBook): boolean {
  const rows = sheetRows(workbook, INCREMENTAL_SHEETS.jobs);
  const code = cellStr(rows[0]?.["Job Code"]);
  return code.startsWith("V3.12-");
}

export function normalizeV312JobCode(raw: string): string {
  const code = cellStr(raw);
  if (code.startsWith("V3.12-")) return `JOBS-${code.slice(6)}`;
...
function normalizeV312EntityId(id: string, entity: keyof typeof MASTER_ENTITY_CODES): string {
  const trimmed = cellStr(id);
  if (trimmed.startsWith("V3.12-")) {
...
function normalizeV312JobRows(rows: Array<Record<string, unknown>>): Array<Record<string, unknown>> {
  return rows.map((row) => {
    const jobId = normalizeV312JobCode(cellStr(row["Job Code"]));
    const systemCode = cellStr(row["System Code"]);
    const ids = idsFromJobId(jobId);
    const machinery = cellStr(row["Machinery Family"]) || V312_MACHINERY_FAMILY;
    ...
    return {
      ...
      "Responsible Vessel Role": cellStr(row["PIC"]) || "Chief Engineer",
      ...
      "Budget Category": cellStr(row["Section"]) || "Inert Gas / IGG / Scrubber",
...
function parseV312Measurements(
  rows: Array<Record<string, unknown>>,
  templateIdByJobId: Map<string, string>,
): ParsedMeasurementRow[] {
  const parsed: ParsedMeasurementRow[] = [];
  for (const [index, row] of rows.entries()) {
    const measurementId = normalizeV312EntityId(cellStr(row["Measurement ID"]), "MEAS");
    const jobId = normalizeV312JobCode(cellStr(row["Linked Job Code"]));
...
function parseV312Checklist(
  ...
  for (const [index, row] of rows.entries()) {
    const checklistItemId = normalizeV312EntityId(cellStr(row["Checklist ID"]), "INSP");
    const jobId = normalizeV312JobCode(cellStr(row["Linked Job Code"]));
...
function parseV312Rfq(rows: Array<Record<string, unknown>>): ParsedRfqRow[] {
  return rows
    .map((row, index) => {
      const jobId = normalizeV312JobCode(cellStr(row["Linked Job Code"]));
      if (!jobId) return null;
      const mappingId = `RFQM-${jobId.replace(/^JOBS-/, "")}`;
      const budgetCategory = cellStr(row["Budget Category"]) || "Inert Gas / IGG / Scrubber Jobs Cost";
      return {
        rowNumber: index + 2,
        mappingId,
        jobId,
...
function synthesizeV312EquipmentMaster(jobs: ParsedMasterJobRow[]): ParsedEquipmentMasterRow[] {
...
function synthesizeV312ComponentMaster(jobs: ParsedMasterJobRow[]): ParsedComponentMasterRow[] {
  const seen = new Set<string>();
  ...
  for (const [index, job] of jobs.entries()) {
    ...
    rows.push({
      ...
      componentType: "Inert Gas System",
      activeFlag: true,
      system: job.systemGroup,
      owner: null,
    });
...
function buildV312RepositoryIndex(jobs: ParsedMasterJobRow[]): V3RepositoryIndexRow[] {
  ...
  return [...bySystem.entries()].map(([systemCode, entry]) => ({
    ...
    status: "Completed",
    machineryFamily: V312_MACHINERY_FAMILY as V3RepositoryIndexRow["machineryFamily"],
  }));
}

export function parseV312IncrementalRepositoryBuffer(buffer: ArrayBuffer | Uint8Array): ParsedV3MasterRepository {
  const workbook = XLSX.read(buffer, { type: "array" });
  if (!isV312IncrementalWorkbook(workbook)) {
    throw new Error("Not a V3.12 incremental EMDR repository workbook");
  }

  const jobRows = normalizeV312JobRows(sheetRows(workbook, INCREMENTAL_SHEETS.jobs));
  ...
  const parsed: ParsedMtilWorkbook = {
    ...
    masterJobs,
    templates,
    measurements: parseV312Measurements(
      sheetRows(workbook, INCREMENTAL_SHEETS.measurements),
      templateIdByJobId,
    ),
    checklistItems: parseV312Checklist(
      sheetRows(workbook, INCREMENTAL_SHEETS.inspections),
      templateIdByJobId,
    ),
    scopeSteps,
    attachments: [],
    spares: parseV312Spares(sheetRows(workbook, INCREMENTAL_SHEETS.spares), templateIdByJobId),
    rfqMappings: parseV312Rfq(sheetRows(workbook, INCREMENTAL_SHEETS.rfq)),
    workflows,
  };
  ...
  return {
    ...normalized,
    emdrMasterData: {
      equipmentMaster: synthesizeV312EquipmentMaster(normalized.masterJobs),
      componentMaster: synthesizeV312ComponentMaster(normalized.masterJobs),
      tools: parseV312Tools(sheetRows(workbook, INCREMENTAL_SHEETS.tools), templateIdByJobId),
    },
    repositoryIndex: buildV312RepositoryIndex(normalized.masterJobs),
    release: EMDR_V312_RELEASE,
  };
}

export function parseV312IncrementalRepositoryFile(path: string): ParsedV3MasterRepository {
  const bytes = fs.readFileSync(path);
  return parseV312IncrementalRepositoryBuffer(bytes);
}

export function parseV312IncrementalRepositoryIfExists(path: string): ParsedV3MasterRepository | null {
  if (!fs.existsSync(path)) return null;
  try {
    return parseV312IncrementalRepositoryFile(path);
  } catch {
    return null;
  }
}
