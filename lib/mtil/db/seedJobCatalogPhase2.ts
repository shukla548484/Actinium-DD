...
import { getAllPhase2Templates } from "@/lib/mtil/phases/phase2/templateCatalog";
import { generatePhase2JobDefinitions } from "@/lib/mtil/phases/phase2/generate";
import {
...
async function upsertTemplate(def: ReturnType<typeof getAllPhase2Templates>[number]) {
...
async function syncMasterJobs() {
  ...
  const nodes = await prisma.jobLibraryNode.findMany({
    where: { nodeType: "standard_job", mtilPhase: 2, deletedAt: null },
...
export async function seedJobCatalogPhase2(): Promise<{
  templates: number;
  masterJobs: number;
  scopeSteps: number;
  ...
  const existing = await prisma.jobDynamicTemplate.count({
    where: { templateId: { startsWith: "TMP-AUX-AE-" }, version: LIBRARY_VERSION },
  });

  await ensureStandardWorkflow();

  const templates = getAllPhase2Templates();
  for (const def of templates) {
    await upsertTemplate(def);
  }

  const scopeSteps = await prisma.jobScopeStep.count({
    where: { templateId: { startsWith: "TMP-AUX-" } },
  });
  const spareMappings = await syncMasterJobs();

  return {
    templates: templates.length,
    masterJobs: generatePhase2JobDefinitions().length,
    scopeSteps,
    spareMappings,
...
export async function isJobCatalogPhase2Seeded(): Promise<boolean> {
  const count = await prisma.jobDynamicTemplate.count({
    where: { templateId: "TMP-AUX-AE-0001", activeFlag: true },
  });
  return count > 0;
}

export async function getJobCatalogPhase2Stats() {
  const [templates, masterJobs, measurements, checklistItems, scopeSteps, spareMappings] =
    await Promise.all([
      prisma.jobDynamicTemplate.count({ where: { templateId: { startsWith: "TMP-AUX-" } } }),
      prisma.masterJobLibrary.count({ where: { jobId: { startsWith: "JOB-AUX-" } } }),
      prisma.jobMeasurement.count({ where: { templateId: { startsWith: "TMP-AUX-" } } }),
      prisma.jobChecklistItem.count({ where: { templateId: { startsWith: "TMP-AUX-" } } }),
      prisma.jobScopeStep.count({ where: { templateId: { startsWith: "TMP-AUX-" } } }),
      prisma.jobSpareMapping.count({ where: { jobId: { startsWith: "JOB-AUX-" } } }),
    ]);
