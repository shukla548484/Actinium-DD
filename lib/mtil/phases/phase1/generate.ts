...
import { PHASE1_MAIN_PROPULSION_SYSTEMS } from "./mainPropulsionSystems";

const PHASE = 1 as const;
const DEPARTMENT = "Main Propulsion";

/** Expand component × action matrix into canonical MTIL job rows. */
export function generatePhase1JobDefinitions(): MtilJobDefinition[] {
  const jobs: MtilJobDefinition[] = [];
  const seqByKey = new Map<string, number>();
  let jobSeq = 0;

  for (const system of PHASE1_MAIN_PROPULSION_SYSTEMS) {
    for (const machinery of system.machinery) {
      for (const component of machinery.components) {
        const templateKey = component.dynamicTemplateKey ?? "me_general_inspect";
        ...
        for (const action of component.actions) {
          ...
          const jobId = buildJobId("ENG", "ME", jobSeq);
...
/** Build JobLibrary tree for Phase 1 main propulsion. */
export function generatePhase1JobLibraryTree(): JobLibrarySeedNode & {
  ...
  const systemsMap = new Map<
    string,
    {
      system: (typeof PHASE1_MAIN_PROPULSION_SYSTEMS)[0];
      machinery: Map<string, { machinery: (typeof PHASE1_MAIN_PROPULSION_SYSTEMS)[0]["machinery"][0]; components: Map<string, MtilJobDefinition[]> }>;
    }
  >();
  ...
  for (const job of jobs) {
    ...
    if (!sys) {
      const systemDef = PHASE1_MAIN_PROPULSION_SYSTEMS.find((s) => s.code === job.systemCode)!;
  ...
  return {
    code: "mtil_p1_main_propulsion",
    name: "Main Propulsion Systems (MTIL Phase 1)",
    nodeType: "department",
    department: DEPARTMENT,
    description: `MTIL Phase 1 v0.2 — ${jobs.length} generated standardized jobs`,
    children: [
      {
        code: "main_propulsion",
    ...
    mtilMeta: { phase: 1, generatedJobCount: jobs.length },
  };
}
...
export function getPhase1Stats() {
  ...
  const catalogTemplates = new Set(
    jobs.map((j) => j.dynamicTemplateKey).filter((k) => !k.startsWith("me_general")),
  );
  return {
    phase: 1,
    engineVersion: "0.2.0",
    ...
    systemCount: PHASE1_MAIN_PROPULSION_SYSTEMS.length,
